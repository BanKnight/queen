...
export class WorkerApplication
{
    ...
    router(template, { index, key })
    {
        this.routers[template] = { index, key, template, ants: {} }
    }
    ...
    async on_spawn(from, { template, inputs, options })
    {
        ...
        const router = this.routers[template]
        if (router)
        {
            const key = router.key(inputs)

            router.ants[key] = router
        }
