...
module.exports = function (context)
{
    ...
    parentPort.on('message', (event) =>
    {
        if (config.debug)
        {
            console.log(`worker[${workerData.index}]:recv_message(queen,${event.type},...)`)
        }
...
function send(worker, event, transfers)
{
    if (typeof worker == "number")
    {
        ...
        {
            send(workers[index], event, transfers)
        }
    }
...
function on_message(worker, name, args)
{
    if (config.debug)
    {
        console.log(`worker[${workerData.index}]:on_message`, name, args)
    }
...
async function on_call(worker, args_alias)
{
    ...
    if (config.debug)
    {
        console.log(`worker[${workerData.index}]:on_call`, name, id)
    }
...
function on_connect(queen, args)
{
    ...
    port.on('message', (event) =>
    {
        if (config.debug)
        {
            console.log(`worker[${workerData.index}]:on_message(${index},${event.type},...)`)
        }
...
function on_broad_ants(worker, args_alias)
{
    ...
    for (const target in ants)
    {
        ...
        try
        {
            invoke_ant(ant.id, from, name, args)
        }
        catch (error)
        {
            console.error(error)
...
function on_spawn(worker, args_alias)
{
    ...
    if (config.debug)
    {
        console.log("spawn", ant.id, template, args, options)
    }
