...
{
    return v8.deserialize(v8.serialize(target))
}
...
function on_broad(worker, event)
{
    ...
    for (const target in ants)
    {
        try
        {
            ...
            post(target, from, name, ...clone_args)
        }
        catch (error)
        {
            console.error(error)
...
function broad(from, name, ...args)
{
    ...
    setImmediate(on_message.bind(QUEEN), {
        type: "broad",
        from,
        target,
        name,
