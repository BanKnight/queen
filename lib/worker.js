...
let workers = new Map()
let ants = new Map()
let worker_agent = null            //agent对应的id
...
function on_message(worker, name, args)
{
    ...
    switch (name)
    {
        ...
        case "stats": return on_stats(worker, args);
        case "quit": return on_quit(worker, args);
...
async function on_start(queen)
{
    ...
    try
    {
        worker_agent = await on_spawn(queen, config.worker)
    }
    catch (error)
    {
        if (config.debug)
...
function on_invoke_ant(worker, args_alias)
{
    ...
    {
        throw new Error("ant not found:" + target + ":" + name)
    }
}
/**
 * 触发ant身上的函数
...
function on_broad_ants(worker, args_alias)
{
    ...
    {
        if (target == worker_agent)        //跳过agent
        {
            continue
        }
 ...
 * @param {*} event 
 */
async function on_destroy(worker, args)
{
    let { id } = args
    ...
    if (ants.size == 0)
    {
        setImmediate(async () =>
        {
            ...
            console.log("all workers ants size is 0,process.exit(0) in 1 second")
...
function broad_ants_method(from, name, args)
{
    ...
    {
        let name_ = name.substr(1)

        send_self("broad_ants_method", {
            from,
            name: name_,
            args
        })
...
function new_ant(id, template, args, options)
{
    ...
    let names = ["info", "log", "warn", "error", "debug", "trace", "table", "timeEnd"]

    for (let key in console)
    {
        if (names.includes(key))
        {
            ant.console[key] = console[key].bind(console, `${ant.template}[${id}]`)
        }
        else
        {
            ant.console[key] = console[key]
        }
    }
